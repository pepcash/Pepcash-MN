...
RELEASEFILE="https://github.com/pepcash/pepcashcore/releases/download/v0.12.1.5/pepcashcore-0.12.1.5-linux64.tar.gz"
SENTINELGIT="https://github.com/pepcash/sentinelLinux.git" # leave empty if coin has no sentinel

daemon="pepcashd"
cli="pepcash-cli"
stopcli="stop"
archive_path="pepcashcore-0.12.1.5/bin/"
core_dir=".pepcashcore"
config_path="$core_dir/pepcash.conf"
node_user="pepcash"
mainnet="14444"
...
function stop_services {
	...
	systemctl stop pepcashd-reindex
	systemctl stop pepcashd
	sleep 3
	killall -9 pepcashd 2>/dev/null
}
...
function install_cronjob {
	...
	crontab -l | { cat; echo "*/15 * * * * (( (\$(curl -s http://explorer.pepcash.com/api/getblockcount) - \$(pepcash-cli getblockcount)) > 10 )) && systemctl restart pepcashd-reindex" ;} | crontab -
}

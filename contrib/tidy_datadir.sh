...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete PepCash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no PepCash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old PepCash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected PepCash 0.7 datadir."
    ;;
  3)
    echo "Detected PepCash pre-0.8 datadir."
    ;;
  4)
    echo "Detected PepCash 0.8 datadir."
    ;;
esac

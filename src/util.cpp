...
const char * const BITCOIN_CONF_FILENAME = "pepcash.conf";
const char * const BITCOIN_PID_FILENAME = "pepcashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pepcash" is a composite category enabling all PepCash-related debug output
            if(ptrCategory->count(string("pepcash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PepCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PepCashCore
    // Mac: ~/Library/Application Support/PepCashCore
    // Unix: ~/.pepcashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PepCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PepCashCore";
#else
    // Unix
    return pathRet / ".pepcashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pepcash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pepcash.conf

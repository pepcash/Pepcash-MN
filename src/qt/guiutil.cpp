...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PepCash address (e.g. %1)").arg("CQVvJdckDT66H99FVZ8LVaFQFpDLMv2NVW"));
#endif
...
{
    // return if URI is not valid or is no pepcash: URI
    if(!uri.isValid() || uri.scheme() != QString("pepcash"))
        return false;
...
{
    // Convert pepcash:// to pepcash:
    //
    //    Cannot handle this later, because pepcash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pepcash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "pepcash:");
    }
...
{
    QString ret = QString("pepcash:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open pepcash.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PepCash.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PepCash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("PepCash (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for PepCash*.lnk
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "pepcash.desktop";
    return GetAutostartDir() / strprintf("pepcash-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a pepcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=PepCash\n";
        else
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PepCash Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PepCash Core app to startup item list
